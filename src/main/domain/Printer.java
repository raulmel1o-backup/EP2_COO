    ...
    private static void printAvailability(LocalDate start, List<Guest> guests, Map<LocalDate, Map<LocalDateTime, List<Guest>>> availability) {
        ...
        for (LocalDateTime dateTimeKey : dateTimeKeys) {
            ...
            for (LocalDate dateKey : dateKeys) {
                ...
                } else if (availability.get(dateKey).get(LocalDateTime.of(dateKey, dateTimeKey.toLocalTime())).size() < guests.size()) {
        ...
        System.out.println("\nHORÁRIOS IDEAIS\n");
        printOptimalTimes(optimalDateTimes);
    }

    private static void printOptimalTimes(List<LocalDateTime> localDateTimes) {
